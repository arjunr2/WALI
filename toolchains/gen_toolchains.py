...
def main():
    ...
    # Generate Fish env
    # -------------------
    fish = textwrap.dedent(f"""\
        #!/usr/bin/env fish
        # Auto-generated by gen_configs.py — source this file

        set -l current_file (status --current-filename)
        set -gx WALI_ROOT_DIR (dirname (realpath $current_file))/{root_dir_rel}
        set -gx WALI_BUILD_DIR $WALI_ROOT_DIR/{build_dir}

        # Build Artifact Directories
        set -gx WALI_LLVM_DIR $WALI_BUILD_DIR/{llvm_dir}
        set -gx WALI_SYSROOT_DIR $WALI_BUILD_DIR/{sysroot}
        set -gx WALI_LIBCXX_DIR $WALI_BUILD_DIR/{libcxx}

        # LLVM Tools
        set -gx WALI_LLVM_BIN_DIR $WALI_LLVM_DIR/bin
        set -gx WALI_CC $WALI_LLVM_BIN_DIR/{cc}
        set -gx WALI_CXX $WALI_LLVM_BIN_DIR/{cxx}
        set -gx WALI_LD $WALI_LLVM_BIN_DIR/{ld}
        set -gx WALI_AR $WALI_LLVM_BIN_DIR/{ar}
    ...
    (filepath / "wali.fish").write_text(fish)
