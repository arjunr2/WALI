...
__attribute__((__import_module__("wali"), __import_name__("SYS_accept4")))
long __imported_wali_accept4(int sockfd, struct sockaddr *addr, socklen_t *addrlen, int flags);
...
int wali_accept4(int sockfd, struct sockaddr *addr, socklen_t *addrlen, int flags) { return (int)__imported_wali_accept4(sockfd, addr, addrlen, flags); }
...
int wali_accept4(int sockfd, struct sockaddr *addr, socklen_t *addrlen, int flags) { return syscall(SYS_accept4, sockfd, addr, addrlen, flags); }
...
int test(void) {
    ...
    int asock = wali_accept4(lsock, (struct sockaddr*)&client_addr, &client_len, SOCK_CLOEXEC);
    ...
    int flags = -1;
    if (asock >= 0) {
        flags = wali_fcntl(asock, F_GETFD, 0);
    }
